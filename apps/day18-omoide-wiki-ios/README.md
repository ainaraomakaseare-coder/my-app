...
- **声で答える機能**：アプリの中のブラウザ（WKWebView）の音声認識は、1回目のあと聞き取らなくなるため、iPhone本体の音声認識をプラグイン（`@capacitor-community/speech-recognition`）で使っています。Web版はこれまでどおりブラウザの音声認識です
...
**必ず「内部テスト」を使ってください。** 「外部テスト」のグループにビルドを入れると、Appleの「ベータ版審査」が終わるまでTestFlightに出てきません（2026-09-25に実際に発生。最初のビルドは審査に1日ほどかかることもある）。内部テストは審査なしで、処理が終わればすぐ配れます。「ベータ版Appの説明」の入力が求められるのも外部テストのときだけで、内部テストなら空欄でも配れます。内部テストで配れるのは、App Store Connectの「ユーザとアクセス」に登録したメンバー（最大100人）です。
