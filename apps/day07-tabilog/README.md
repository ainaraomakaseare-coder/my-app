...
Googleログインを設定しても、**このリンクを開いて見たり書き足したりすること自体にログインは要りません。** ログインが必要なのは「評価（★）を付ける」「マイログを見る」ときだけです。ログインした本人の名前が「記録した人」に自動で入るようになります（手で書き直すこともできます）。**リンクの共有自体は今までどおりで、招待の仕組みが変わるわけではありません。** 「＋ メンバーを招待」ボタンも同じ意味（このURLをコピーして送る）のまま残しています。

## Googleログインについて（任意設定）

Googleアカウントでのログインを追加できます。**設定しなくても、今までどおりリンク共有だけで使えます。**（`index.html` の `tabilog-google-client-id` メタタグが空のままなら、ログイン機能自体が無効になり、評価・マイログの画面も出ません。）設定した場合も、閲覧・記録の追加にログインは必須になりません。ホーム画面の「ログインする」ボタン、または記録の評価欄から、必要なときだけログイン画面へ行けます。
