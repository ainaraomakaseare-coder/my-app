...
test('bundle contains only web entry and bridge, with no external fonts',async()=>{
 assert.deepEqual((await fs.readdir('www')).sort(),['index.html','native.js']);
...
test('native config bundles offline assets and never a remote live-reload server',async()=>{
 const c=JSON.parse(await fs.readFile('capacitor.config.json','utf8'));assert.equal(c.appId,'com.hiroyaapps.baydiary');assert.equal(c.webDir,'www');assert.equal(c.server,undefined);
});
