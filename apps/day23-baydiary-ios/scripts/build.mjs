...
await fs.copyFile('resources/icon.png','www/icon.png');
await fs.writeFile('www/index.html',html);
const names=(await fs.readdir('www')).sort();
if(JSON.stringify(names)!==JSON.stringify(['icon.png','index.html','native.js'])) throw new Error('Unexpected bundled files: '+names.join(', '));
