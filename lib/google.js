...
async function accessTokenFor(account, db) {
  ...
  // 期限まで2分を切っていたら取り直す。その前に、ほかの処理が取り直していないか DB を見る。
  if (health.usable(account)) return account.access_token;
  await health.reload(account, db);
  if (health.usable(account)) return account.access_token;

  let fresh;
  try {
    fresh = await refresh(account.refresh_token);
  } catch (e) {
    // ★ 切れた理由（invalid_grant など）を連携設定の画面に残す（lib/token-health.js）。
    await health.noteAuthError(account, db, e);
    throw e;
  }
  const patch = {
    access_token: fresh.access_token,
    expires_at: new Date(Date.now() + (fresh.expires_in || 3600) * 1000).toISOString(),
    ...(account.meta && account.meta.auth_error ? { meta: health.clearAuthError(account.meta) } : {}),
  };
