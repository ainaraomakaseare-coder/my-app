 ...
 */
async function accessTokenFor(network, db) {
  const row = await db.getToken(network);
  ...
  await db.saveToken(network, { access_token: fresh.access_token, expires_at: expiresAt });
