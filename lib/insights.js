...
// Threads
//
// ★ 毎晩ここを通るたびに、トークンの延長（lib/threads.js）も一緒に行われる。
//   数字を取るついでに、60日で切れないようにしている。
// ---------------------------------------------------------------------------

async function thAccountStats(account, db) {
  try {
    const token = await threads.accessTokenFor(account, db);
    const userId = account.external_id || 'me';
    const raw = await threads.send('GET', `/${threads.VERSION}/${userId}/threads_insights`, {
      metric: 'followers_count', access_token: token,
    });
    // 返事は { data: [{ name:'followers_count', total_value:{ value: 12 } }] } の形
    const row = (raw.data || []).find((d) => d.name === 'followers_count') || {};
    const value = row.total_value ? row.total_value.value : null;
    return {
      ok: true,
      metrics: { followers: numOrNull(value), views: null, likes: null, posts: null },
      raw,
    };
  } catch (e) {
    return failFrom(e);
...
async function thPostStats(account, targets, db) {
  try {
    const token = await threads.accessTokenFor(account, db);
    const list = (targets || []).filter((t) => t.external_id && (!t.network || t.network === 'threads'));
    const byTargetId = {};
    const raw = {};
    for (const t of list) {
      try {
        const got = await threads.send('GET', `/${threads.VERSION}/${t.external_id}/insights`, {
          metric: 'views,likes,replies,reposts,quotes,shares', access_token: token,
        });
        raw[t.id] = got;
        // 返事は { data: [{ name:'views', values:[{ value: 120 }] }, ...] } の形
        const pick = (name) => {
          const m = (got.data || []).find((d) => d.name === name);
          const v = m && (m.values && m.values[0] ? m.values[0].value : m.total_value && m.total_value.value);
          return numOrNull(v);
        };
        byTargetId[t.id] = {
          views: pick('views'),
          ...
          shares: pick('shares'),
          extra: { reposts: pick('reposts'), quotes: pick('quotes') },
        };
      } catch (e) {
        // ★ この投稿だけ失敗しても、他の投稿の取り込みは止めない。
        raw[t.id] = e.raw || null;
...
async function accountStats(account, db) {
  ...
  switch (account.network) {
    ...
    case 'threads': return thAccountStats(account, db);
    default:
      return {
        ok: false,
...
async function postStats(account, targets, db) {
  ...
  switch (account.network) {
    ...
    case 'threads': return thPostStats(account, targets, db);
    default:
      return {
        ok: false,
