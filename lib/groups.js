...
async function create(body) {
  ...
  // ★ 既定は「自動投稿しない」。新しい運用ラインは安全な側から始める。
  const auto_publish = body.auto_publish === true;
  ...
  return { group: await db.insertGroup({ label, validation_profile, auto_publish }) };
}
...
async function update(id, body) {
  ...
  if (body.auto_publish !== undefined) patch.auto_publish = body.auto_publish === true;
...
module.exports = {
  MAX_LABEL, UUID, bad,
  normalizeLabel, normalizeProfile, profileChoices,
