...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (q.creator)               return res.status(200).json(await tiktokCreator(q.creator));
    ...
    if (network === 'threads') return await oauthThreads(req, res, code);

    return res.status(400).json({ error: `${network} には対応していません。` });
  } catch (err) {
    if (req.method === 'POST' || req.method === 'DELETE' || req.method === 'PATCH' || q.creator) {
...
const NET_LABEL = {
  youtube: 'YouTube', instagram: 'Instagram', x: 'X', tiktok: 'TikTok', threads: 'Threads',
};

// ---------------------------------------------------------------- Threads

async function oauthThreads(req, res, code) {
  const redirectUri = selfUrl(req, 'threads');
  if (!code) return go(res, threads.authUrl(redirectUri, crypto.randomBytes(8).toString('hex')));

  // code → 短期 → 長期（60日）まで一気に引き換える。延長は lib/threads.js が自動で行う。
  const token = await threads.exchangeCode(code, redirectUri);
  const who = await safe(() => threads.me(token.access_token), 'Threads');
  const groupId = await resolveGroup(takeGroup(req, res));

  await saveAccount({
    network: 'threads', name: who.name, externalId: who.id || token.user_id, groupId,
    token: {
      access_token: token.access_token,
      refresh_token: null,
      expires_at: token.expires_at,
      // 延ばせるのは発行から24時間たってから。その起点を残す。
      meta: { issued_at: new Date().toISOString() },
    },
  });
  return back(res, `Threads「${who.name}」と接続しました（トークンは自動で延長されます）`);
}
...
async function oauthYoutube(req, res, code) {
  ...
  await saveAccount({
    ...
    token: {
      ...
      // 繋ぎ直したので、前に切れたときの記録（lib/token-health.js）は消す。
      meta: {},
    },
  });
...
async function oauthTiktok(req, res, code) {
  ...
  await saveAccount({
    ...
    token: {
      ...
      // ★ 実際にもらえた権限を残す。直接投稿できるかはこれで決める。
      meta: tiktok.withScopes(null, token.scope),
    },
  });
  const direct = tiktok.parseScopes(token.scope).includes('video.publish');
  return back(res, direct
    ? `TikTok「${who.name}」と接続しました（直接投稿できます。投稿ごとに公開範囲などを選んでください）`
    : `TikTok「${who.name}」と接続しました（動画はTikTokアプリの下書きに届きます。本文は一緒に送れないので、公開のときに貼ってください）`);
}

/**
 * 直接投稿の画面に出す、投稿者の情報（名前・公開範囲の選択肢・上限）。
 *
 * ★ TikTok の決まりで、投稿設定を出すたびに最新を取る。
 *   トークンは返さない。返すのは画面に要るものだけ。
 */
async function tiktokCreator(accountId) {
  if (!accountId || !UUID.test(String(accountId))) throw hint('どの連携か指定されていません。');
  const account = await db.getAccount(accountId);
  if (!account || account.network !== 'tiktok') throw hint('TikTok の連携が見つかりません。');
  if (!tiktok.canDirectPost(account)) {
    throw hint('このTikTok連携には直接投稿の権限がありません。',
      '下書き方式で送られます。直接投稿にするには、連携をやり直してください。');
  }
  const token = await tiktok.accessTokenFor(account, db);
  return { creator: await tiktok.creatorInfo(token) };
}
