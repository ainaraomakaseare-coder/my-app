...
async function instagram(req) {
  ...
  // ここで初めて本物か確かめる。
  // 失敗したときは、Meta が返したコードと、貼られた値の見た目を添える。
  // これが無いと「権限がありません」だけが出て、
  // 貼り間違いなのか設定の問題なのかを切り分けられない。
  let who;
  try {
 ...
 * 自分自身のコールバックURL。
 *
 * ★ 「?network=youtube」のようなクエリ付きにはしない。
 *   Google・TikTok・X は、登録するリダイレクトURIにクエリが付いていると
 *   受け付けないことがある。パスに含める形なら3社とも通る。
 */
