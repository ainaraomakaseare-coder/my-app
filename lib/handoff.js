...
const PUBLISHING_NETWORKS = ['instagram', 'x', 'threads'];
...
const DELIVERY = {
  ...
  threads: {
    mode: 'hand',
    action: '本文（と画像・動画）を受け取る',
    stops: 'Threads は API で送ると即公開になります。本文をコピーして、アプリから投稿してください。',
    needs: ['thText'],
    note: 'A8.net は Threads の投稿本文への案件リンク掲載を控えるよう案内しているため、案件リンクは載せられません。',
  },
  x: {
    mode: 'hand',
    action: '画像2枚と本文を受け取る',
...
/**
 * TikTok を直接投稿で出すときの渡し方。
 *
 * ★ 直接投稿できる連携（video.publish あり）で、投稿に本人の投稿設定が
 *   入っているときだけこちらになる。本文も一緒に送るので、
 *   公開のときに本文を貼る手間（ttCaption）は残らない。
 *   判定は lib/networks/tiktok.js と同じ条件にそろえる。
 */
const TIKTOK_DIRECT = {
  mode: 'api',
  action: 'TikTokに直接投稿する',
  stops: '本文と公開範囲も一緒に送り、そのまま公開されます。',
  needs: [],
  note: '審査前のアプリは、公開アカウントへは直接投稿できません。そのときは自動で下書き送信に切り替わります（本文は手で貼ってください）。',
};

// ★ 直接投稿のつもりでも、審査前で下書き送信に切り替わることがある。
//   そのときは本文を貼る手間が残るので、実際の送り方（stage）を優先する。
//   stage の名前は lib/networks/tiktok.js が書く。
const INBOX_STAGES = ['uploaded', 'published_inbox'];

function deliveryOf(account, post, target) {
  const net = account && account.network;
  if (net !== 'tiktok') return DELIVERY[net];
  if (target && INBOX_STAGES.includes(target.stage)) return DELIVERY.tiktok;
  return account.can_direct_post && post && post.tt_settings ? TIKTOK_DIRECT : DELIVERY.tiktok;
}
...
const ASSET_READY = {
  ...
  thText: (p) => !!(p.th_text || p.body_common || '').trim(),
};
...
/**
 * 送信が終わったあとも、人の手が残っている投稿先。
 *
 * ★ TikTok は「下書きに届く」で終わりではない。
 *   下書き方式（inbox）のリクエストは source_info しか受け取らないので、
 *   本文を一緒に送れない。TikTok 側は自前の既定値（アプリ名）を入れるため、
 *   何もしないと「#toukoutaku neo」のような文言のまま公開されてしまう。
 *
 *   つまり「送信成功」と「投稿が仕上がった」は別。公開のときに本文を貼る
 *   作業が残る。ここを返して、投稿カードの上で本文を取り出せるようにする。
 *
 * ★ 受け渡しの画面は下書きしか並べないので、予約が走ったあとは本文に
 *   たどり着けなくなっていた。いちばん要るのが「TikTokを開いた直後」
 *   なのに、そこで消えていたのが実際の詰まりだった。
 */
function afterSend(plan) {
  return (plan || []).filter((row) =>
    row && row.mode === 'api' && row.status === 'success' &&
    Array.isArray(row.needs) && row.needs.length > 0);
}
...
module.exports = {
  PUBLISHING_NETWORKS, DELIVERY, TIKTOK_DIRECT, ASSET_LABEL, ASSET_READY,
  MANUAL, HANDED, OFF_PIPELINE,
  isPublishing, deliveryFor, autoPublishesTo, statusForTarget, manualOnly, planFor, afterSend,
};
