 ...
 *
 * ★ 止めるかどうかは運用アカウントごと。
 *   ひろや側はいままでどおり自動投稿してよい。
 *   案件側は §3-1（自動投稿はしない）に従って止める。
 */

/** 叩いた瞬間に公開されてしまうSNS。schema_v4 の publishing_networks() と同じ。 */
 ...
 *   ここを安全側（false）に倒すと、列を足した直後に
 *   動いている予約投稿が全部止まってしまう。
 *   止める判断は、運用アカウントで明示的に設定されたときだけ。
 */
const autoPublishes = (group) => !group || group.auto_publish !== false;
...
function statusForTarget(group, network) {
  return autoPublishes(group) || !isPublishing(network) ? 'queued' : 'manual';
}

/** 予約しようとしている投稿先のうち、手渡しにしかできないもの。 */
function manualOnly(group, accounts) {
  if (autoPublishes(group)) return [];
  return (accounts || []).filter((a) => a && isPublishing(a.network));
}
...
function planFor(post, accounts, group, targets) {
  ...
  return (accounts || []).map((a) => {
    ...
    // 自動投稿する運用アカウントでは、公開系も API で出してよい。
    const mode = d && d.mode === 'hand' && autoPublishes(group) ? 'api-publish' : (d ? d.mode : 'hand');
...
module.exports = {
  ...
  isPublishing, deliveryFor, autoPublishes, statusForTarget, manualOnly, planFor,
};
