...
function collectionFindings(accounts) {
  ...
  //
  // ★ 原因を決めつけない。
  //   YouTube は「登録者数を非公開」にしていると、権限が足りていても返ってこない。
  //   それを「権限が足りません」と言うと、直しようのない指示になる。
  //   SNSごとに、実際にありうる原因だけを挙げる。
  for (const a of accounts) {
    ...
    out.push(say('collection', BLOCKED,
      ...
      missingWhy(a, missing),
      // ★ SNS側が理由を言っているなら、それをそのまま見せる。こちらの推測より確か。
      a.latest.error ? ['SNS側の返事：' + a.latest.error] : []));
  }
  return out;
}

/**
 * その SNS で、実際にありうる原因だけを挙げる。
 *
 * ★ 欠けた項目まで見る。
 *   YouTube の「非公開にしていると取れない」はフォロワー数の話であって、
 *   総再生数には当てはまらない。欠けた項目を見ずに同じ文言を返すと、
 *   「総再生数が返ってきません → 登録者数を公開してください」という
 *   噛み合わない案内になる。直しに行っても直らない。
 */
function missingWhy(a, missing) {
  const lacks = missing || [];
  const lacksFollowers = lacks.includes('フォロワー数');

  if (a.network === 'youtube') {
    if (lacksFollowers) {
      return 'YouTube は、チャンネル登録者数を非公開にしていると返ってきません。'
        + 'それが理由なら、YouTube Studio の設定 → チャンネル → 詳細設定 で公開にすると取れます。'
        + '公開しているのに出ない場合は、連携を繋ぎ直してみてください。';
    }
    return 'YouTube が、この項目を返していません。'
      + '通常は取れる値なので、連携を繋ぎ直しても出ない場合は、'
      + '下の「接続テスト」で YouTube が何を返しているか確かめてください。';
  }
  if (a.network === 'instagram') {
    return 'つながってはいるので、この項目を読む権限が足りていない可能性があります。'
      + 'Instagram はプロアカウント（ビジネス／クリエイター）でないと数字を返しません。'
      + '「連携設定」の接続テストで、Meta が何と言っているか確かめられます。';
  }
  if (a.network === 'tiktok') {
    return 'TikTok は user.info.stats の権限が要ります。'
      + '管理画面（アプリ側とサンドボックス側の両方）で有効にしたうえで、'
      + '連携を繋ぎ直してください。古い引換券には新しい権限が入っていません。';
  }
  return 'つながってはいるので、この項目を読む権限が足りていない可能性があります。';
}
...
module.exports = {
  ...
  collectionFindings, missingWhy, followerFindings, postFindings, stalledFindings,
  observations,
};
