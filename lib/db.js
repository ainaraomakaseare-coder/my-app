...
async function listAccounts() {
  const rows = (await rest('sns_accounts', {
    query: {
      select: 'id,network,label,account_name,external_id,group_id,expires_at,updated_at,refresh_token,meta',
      order: 'network.asc,created_at.asc',
    },
  })) || [];

  // ★ meta もそのままは返さない。画面が要るのは「直接投稿できるか」と
  //   「最後に取り直しに失敗した理由」（lib/token-health.js）だけ。
  return rows.map(({ refresh_token, meta, ...a }) => ({
    ...a,
    // Threads は引換券が無く、長期トークンそのものを自動で延ばす（lib/threads.js）。
    auto_refresh: !!refresh_token || a.network === 'threads',
    can_direct_post: a.network === 'tiktok' && !!(meta && Array.isArray(meta.scopes) && meta.scopes.includes('video.publish')),
    auth_error: (meta && meta.auth_error) || null,
  }));
}
...
async function listGroups() {
  return (await rest('account_groups', {
    query: { select: 'id,label,validation_profile,auto_publish_networks,features', order: 'created_at.asc' },
  })) || [];
}
...
/**
 * いま繋ごうとしている相手が、もう連携済みかどうかを探す。
 *
 * ★ 見分けるのは external_id（SNS側の変わらないID）。
 *   表示名で見分けていたときは、名前がぶつかると
 *   別チャンネルの連携先を上書きしてしまっていた。
 *   TikTok は表示名が取れないと 'TikTok' に落ちるので、
 *   2つ目を繋いだ瞬間に1つ目を奪う、という壊れ方をしていた。
 *
 * ★ external_id が空の行は v7 より前からあるもの。
 *   そのときだけ、今までどおり名前で突き合わせて拾う。
 *   拾えば次の保存でIDが入り、以後はぶつからなくなる。
 */
async function findAccountByIdentity({ network, external_id, account_name }) {
  const select = 'id,network,label,account_name,group_id,external_id';

  if (external_id) {
    const rows = await rest('sns_accounts', {
      query: { select, network: `eq.${network}`, external_id: `eq.${external_id}` },
    });
 ...
 * 連携したときの保存。
 *
 * ★ どの行に上書きするかは、呼ぶ側が findAccountByIdentity で決めて id で渡す。
 *   ここで勝手に探して上書きしない。
 *   「上書きしてよいか」の判断（別チャンネルのものではないか）は
 *   lib/connect.js が持っていて、ここからは見えないため。
 */
async function upsertAccount({ id, network, ...rest_ }) {
  const patch = { ...rest_, updated_at: new Date().toISOString() };

  if (id) return updateById('sns_accounts', id, patch);
...
module.exports = {
  ...
  listAccounts, getAccount, findAccountByIdentity, upsertAccount, updateAccount, deleteAccount, DbError,
