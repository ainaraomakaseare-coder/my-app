...
async function accessTokenFor(account, db) {
  ...
  // ★ X の引換券は1回使うと取り替わり、古い券はその場で無効になる。
  //   手元の行が古いまま取り直すと、それだけで連携が切れる。先に DB の最新を読む。
  ...
  try {
    ...
    // 切れた理由を連携設定に残す（lib/token-health.js）
  ...
  const patch = {
    ...
    ...(row.meta && row.meta.auth_error ? { meta: health.clearAuthError(row.meta) } : {}),
  };
