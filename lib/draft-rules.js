...
/**
 * 本文としてこれ未満は「無い」のと同じ扱いにする。
 * ★ ハッシュタグだけの投稿（例：「#toukoutakuneo」1語）を、
 *   本文ありと誤判定しないための下限。日本語の短い一文でも
 *   これより短くはならないはずの、余裕を持った値。
 */
const MIN_CAPTION_LEN = 8;
...
/**
 * ★ 0.55 から 0.75 に上げた。
 *   0.55 倍は 32px が 18px になる。縦型動画をスマホで見ると読めない。
 *   しかも6行そろえて縮める作りなので、長い行が1本あるだけで
 *   6行ぜんぶが小さくなる。「文字が小さい」の原因はここだった。
 *
 *   縮みで吸収するのは「少しはみ出した」ぶんだけにして、
 *   それを超えるものは中身の問題として書き直させる。
 */
const MIN_SCALE = 0.75;   // reel.js の READABLE_SCALE と同じ値であること
const MAX_ROW_FIT = Math.floor(MAX_ROW_WIDTH / MIN_SCALE);       // 20
const MAX_TITLE_FIT = Math.floor(MAX_TITLE_WIDTH / MIN_SCALE);   // 18

/** 問いだけの上限。答えの枠が右に寄りすぎると、穴埋めに見えなくなる。 */
const MAX_QUESTION_WIDTH = 11;
...
function validateDraft(draft, profile) {
   ...
   * ★ 本文が空、または短すぎたら止める。
   *
   *   実際に起きた不具合：igCaption が空のまま検査を素通りし、
   *   投稿される本文がハッシュタグだけ（#toukoutakuneo のような1語）になっていた。
   *
   *   原因は、この下と missing-sourcing / missing-pr-label が
   *   `if (text && ...)` という書き方をしていたこと。空文字は falsy なので
   *   「本文が無ければ見ない」がそのまま「合格」に見えてしまっていた。
   *   ここだけは text の有無に関わらず必ず見る。
   */
  for (const [field, text] of texts) {
    const t = String(text == null ? '' : text).trim();
    if (t.length < MIN_CAPTION_LEN) {
      out.push({ field, rule: 'empty-caption', severity: 'error',
        message: t
          ? `本文が短すぎます（${MIN_CAPTION_LEN}字以上）。ハッシュタグだけの投稿にならないよう、中身のある文章にしてください`
          : '本文が空です。ハッシュタグだけの投稿にならないよう、中身のある文章を書いてください',
        excerpt: t });
    }
  }
  ...
  //
  // ★ ここの `text &&` は空文字を弾く役目ではない。空はもう上の
  //   empty-caption が止めている。ここは「非空だが断りが無い」だけを見る。
  ...
  // --- 穴埋めとして成り立っているか ---
  //
  // ★ ここが「赤字と黒字が2回繰り返している」の正体。
  //   問い「職場で話す人ほど広まる」→ 答え「広まる」のように、
  //   答えが問いの中に既に入っていると、同じ語が黒と赤で2回出る。
  //   穴埋めになっていないので、動画として意味をなさない。
  rows.forEach((row, i) => {
    ...
    if (q.includes(a)) {
      out.push({ field: `rows[${i}]`, rule: 'answer-in-question', severity: 'error',
        message: '答えが問いの中にもう出ています。同じ言葉が2回出るので、問いから外してください',
        excerpt: `${q} → ${a}` });
    }

    // ★ 問いが言い切りだと、答えが付け足しに見える。
    //   実物は「〜ほど」「〜すると」のように、次に続く形で切ってある。
    if (/[。．！？!?]$/.test(q)) {
      out.push({ field: `rows[${i}].question`, rule: 'question-not-open', severity: 'error',
        message: '問いが文として終わっています。答えに続く形（「〜ほど」「〜すると」）で切ってください',
        excerpt: q });
    }

    if (visualWidth(q) > MAX_QUESTION_WIDTH) {
      out.push({ field: `rows[${i}].question`, rule: 'question-too-wide', severity: 'warning',
        message: `問いが長いです（${MAX_QUESTION_WIDTH}字ぶんまでが読みやすい）`, excerpt: q });
    }
  });
...
module.exports = {
  ...
  MAX_ROW_WIDTH, MAX_TITLE_WIDTH, MAX_ROW_FIT, MAX_TITLE_FIT, MIN_SCALE, MAX_QUESTION_WIDTH,
  ANSWER_MIN, ANSWER_MAX, ROW_COUNT,
  HASHTAG_MIN, X_HASHTAG_MAX, MIN_CAPTION_LEN, normalizeHashtags, captionWithTags,
};
