...
async function step({ post, target, account, db }) {
  ...
  // ★ Instagram は PNG を受け付けない。
  //   気づくのが Meta 側の変換失敗のあとだと、原因が分かりにくく数分を失う。
  //   送る前に、こちらで止めて理由を言う。
  if (post.media_kind === 'image' && /\.png$/i.test(post.media_path)) {
    throw hint(
      'Instagram は PNG 画像を受け付けません。',
      'JPEG（.jpg）で保存し直してアップロードしてください。スマホで撮った写真はそのまま使えます。'
    );
  }
